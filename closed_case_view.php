...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: closed_case_view.php?id=$id"); // refresh with updated status
    exit;
}

$stmt = $conn->prepare("SELECT case_id, crime_type, description, date_reported, reporter_address, suspect_name, status 
                        FROM cases WHERE case_id=? AND status='Closed'");
...
<head>
    <meta charset="UTF-8">
    <title>Closed Case <?= htmlspecialchars($id) ?></title>
...
<div class="container">
    ...
    <?php else: ?>
    <p>No closed case found.</p>
    <?php endif; ?>

    <a href="closed_cases.php"><button class="btn-danger">Back</button></a>
</div>
</body>
</html>