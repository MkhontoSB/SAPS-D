...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: ongoing_case_view.php?id=$id"); // refresh with updated status
    exit;
}

// Fetch case details (excluding Cold, Closed, Protection Order)
$stmt = $conn->prepare("SELECT case_id, crime_type, description, date_reported, reporter_address, suspect_name, status 
                        FROM cases 
                        WHERE case_id=? AND status='Ongoing'");
...
<head>
    <meta charset="UTF-8">
    <title>Ongoing Case <?= htmlspecialchars($id) ?></title>
...
<div class="container">
    ...
    <?php else: ?>
    <p>No ongoing case found.</p>
    <?php endif; ?>

    <a href="ongoing_cases.php"><button class="btn-danger">Back</button></a>
</div>
</body>
</html>
